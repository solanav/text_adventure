...
void graphic_engine_paint_space(Graphic_engine *ge, Game *game, int position_of_space)
{
	...
	if (position_of_space == 0)
		space_to_use = space_last;
	if (position_of_space == 1)
		space_to_use = space_act;
	if (position_of_space == 2)
		space_to_use = space_next;
	...
	if (position_of_space == 0)
		obj = obj_string_back;
	else if (position_of_space == 1)
		obj = obj_string_act;
	else if (position_of_space == 2)
		obj = obj_string_next;
	...
	if (id_right > 10)
		sprintf(link_right, ">%ld", id_right);
	else if (id_right == -1)
		sprintf(link_right, "   ");
	else
		sprintf(link_right, "> %ld", id_right);
	...
	if (link_id_left > 10)
		sprintf(link_id_left_char, "[%ld]", link_id_left);
	else if (link_id_left == -1)
		sprintf(link_id_left_char, "    ");
	else
		sprintf(link_id_left_char, "[%ld ]", link_id_left);

	if (link_id_right > 10)
		sprintf(link_id_right_char, "[%ld]", link_id_right);
	else if (link_id_right == -1)
		sprintf(link_id_right_char, "    ");
	else
		sprintf(link_id_right_char, "[ %ld]", link_id_right);
	...
	{
		if (position_of_space == 0)
			print_new_line(ge->map, 5);
		if (position_of_space == 2)
			print_new_line(ge->map, 6);
	}
